...
const Product = ({ _id, productName, img, price, color, badge, des }) => {
  const dispatch = useDispatch();

  const idString = productName.toLowerCase().split(" ").join("");
  const navigate = useNavigate();

  const handleProductDetails = () => {
    navigate(`/product/${idString}`, {
      state: {
        item: { _id, productName, img, price, color, badge, des },
      },
    });
  };

  return (
    <div className="w-full max-w-[320px] relative group flex flex-col justify-between">
